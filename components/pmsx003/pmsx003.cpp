...
void PMSX003Component::setup() {
  ...
  //
  // safe_mode calls App.setup() from inside should_enter_safe_mode() and then returns early
  // from the generated setup(). This component is registered before that early return, but
  // set_uart_parent() is emitted after it, so in safe mode the UART parent is null and the
  // first loop() -> send_command_() -> write_byte() panics (LoadProhibited, EXCVADDR 0x0).
  // Failing here also disables loop(), so safe mode stays up.
  //
  // Verified still present in ESPHome 2026.7.4. Remove this guard once fixed upstream.
  if (this->parent_ == nullptr) {
    this->mark_failed(LOG_STR("uart parent not set"));
    return;
  }
}
...
void PMSX003Component::dump_config() {
  // dump_config() runs even for components marked FAILED, and check_uart_settings() below
  // dereferences the UART parent - null in safe mode. See the note in setup().
  if (this->parent_ == nullptr) {
    ESP_LOGCONFIG(TAG, "PMSX003: uart parent not set");
    return;
  }
